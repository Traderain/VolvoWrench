	...
	public class GoldSourceParser
	{
		...
		public static GoldSource.IncludedBXtData FormatBxtData(byte[] bxtdata)
		{
			...
			if (bxtdata.Count()%8 == 0)
			{
				...
				{
					decryptedBxtData.AddRange(Tea.Decrypt(packedbytes.ToArray()));
				}
		...
		{
			var mw = new[] { (byte)'/', (byte)'/', (byte)'B', (byte)'X', (byte)'T', (byte)'D' };
									...
									switch (currentDemoFrame.Type)
									{
										...
										default:
											...
											if(nf.MVars.Gravity != 800)
												gDemo.Cheats.Add("Gravity: " + nf.MVars.Gravity);
											nf.MVars.Stopspeed = br.ReadSingle();
											if (nf.MVars.Stopspeed != 100)
												gDemo.Cheats.Add("Stopspeed: " + nf.MVars.Stopspeed);
											nf.MVars.Maxspeed = br.ReadSingle();
											if (nf.MVars.Maxspeed != 320)
												gDemo.Cheats.Add("Maxspeed: " + nf.MVars.Maxspeed);
											nf.MVars.Spectatormaxspeed = br.ReadSingle();
											nf.MVars.Accelerate = br.ReadSingle();
											if (nf.MVars.Accelerate != 10)
												gDemo.Cheats.Add("Accelerate: " + nf.MVars.Accelerate);
											nf.MVars.Airaccelerate = br.ReadSingle();
											if (nf.MVars.Airaccelerate != 10)
												gDemo.Cheats.Add("Airaccelerate: " + nf.MVars.Airaccelerate);
											nf.MVars.Wateraccelerate = br.ReadSingle();
											nf.MVars.Friction = br.ReadSingle();
											if (nf.MVars.Friction != 4)
												gDemo.Cheats.Add("Friction: " + nf.MVars.Friction);
											nf.MVars.Edgefriction = br.ReadSingle();
											if (nf.MVars.Edgefriction != 2)
												gDemo.Cheats.Add("Edgefriction: " + nf.MVars.Edgefriction);
											...
											if (nf.MVars.Stepsize != 18)
												gDemo.Cheats.Add("Stepsize: " + nf.MVars.Stepsize);
											nf.MVars.Maxvelocity = br.ReadSingle();
											if (nf.MVars.Maxvelocity != 2000)
												gDemo.Cheats.Add("Maxvelocity: " + nf.MVars.Maxvelocity);
