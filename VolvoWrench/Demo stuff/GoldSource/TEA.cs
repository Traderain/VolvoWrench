...
namespace TeaCrypto
{
    ...
    public class Tea
    {
        ...
        /// </summary>
        public static uint[] BxtKey = new uint[] {0x1337FACE, 0x12345678, 0xDEADBEEF, 0xFEEDABCD};

        /// <summary>
        /// Decrypts an array of Uin32s encrypted with BXT's TEA key.
        /// </summary>
        /// <param name="data">The data to decrypt</param>
        public static UInt32[] Decrypt(byte[] data)
        {
            if(data.Length != 8)
                throw new Exception("Invalid data! (Invalid number of bytes supplied)");
            UInt32 v0 = BitConverter.ToUInt32(data.Take(4).ToArray(),0), v1 = BitConverter.ToUInt32(data.Skip(4).ToArray(),0), sum = 0xC6EF3720;
            for (var i = 0; i < 32; i++)
            {
