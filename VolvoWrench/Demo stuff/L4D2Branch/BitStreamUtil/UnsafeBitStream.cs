...
namespace VolvoWrench.Demo_Stuff.L4D2Branch.BitStreamUtil
{
    public unsafe class UnsafeBitStream : IBitStream
    {
        ...
        public bool ReadBit()
        {
            var bit = (PBuffer[Offset >> 3] & (1 << (Offset & 7))) != 0;
        ...
        {
            var iResult = PeekInt(32); // omfg please inline this
            ...
            if ((buf & MSB_1) != 0)
            {
                ...
                if ((buf & MSB_2) != 0)
                {
                    ...
                    if ((buf & MSB_3) != 0)
                    {
                        ...
                        if (TryAdvance(4*8)) RefillBuffer();
                    }
        ...
        private void ReadBytes(byte[] ret, int bytes)
        {
            ...
            else if ((Offset & 7) == 0)
            {
                ...
                while (offset < bytes)
                {
                    var remainingBytes = Math.Min((BitsInBuffer - Offset) >> 3, bytes - offset);
                ...
                fixed (byte* retptr = ret)
                {
                    ...
                    while (offset < bytes)
                    {
                        var remainingBytes = Math.Min(((BitsInBuffer - Offset) >> 3) + 1, bytes - offset);
        ...
        private void HyperspeedCopyRound(int bytes, byte* retptr) // you spin me right round baby right round...
        {
            ...
            var misalign = 8 - (Offset & 7);
            var realign = sizeof (ulong)*8 - misalign;
            ulong step = ReadByte(misalign);
            var inptr = (ulong*) (PBuffer + (Offset >> 3));
            var outptr = (ulong*) retptr;
            // main loop
            for (var i = 0; i < ((bytes - 1)/sizeof (ulong)); i++)
            {
                var current = *inptr++;
